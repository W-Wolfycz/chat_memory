...
| `query_rounds` | `umo, conversation_id, user_id=None, limit_rounds=10, llm_status=None, content_kind=None, persona_id=None, since=None, until=None` | `list[list[dict]]`，每轮 `[user, assistant]` 两条 |
...
- `persona_id` 非空时按 persona 严格过滤（`query_rounds` 的 user + assistant 都加条件，保证配对同 persona；None / 空串跳过）
- `since` / `until` 给定时按 `created_at` 过滤时间窗口（含端点）；`datetime` 为 tz-aware 时自动转 UTC naive，naive 假定已是 UTC（与落库 `CURRENT_TIMESTAMP` 对齐）
- `query_rounds` 的 EXISTS 子查**不**加 persona / 时间条件，保持"有配对"语义——若配对 assistant 落在过滤范围外，该 user 仍被选中但 assistant 查询返回空，结果是 `[user_dict]` 一条
...
`filter_by_persona` 开启后，takeover 查询严格按当前 persona 过滤——切换 persona 时旧 persona 时期的对话自动隔离，切回后自然回归。`persona_id` 为空（如老库 ALTER 补列后的旧行）会跳过过滤避免老数据全被滤光。
...
- 老库（v2.3.3）启动时自动 `ALTER TABLE ADD COLUMN persona_id TEXT` 补列，旧行 persona_id 为 NULL
- 入库时 `persona_id` 取自 `req.conversation.persona_id`（on_llm_request 时已就绪）
